...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run SCC no-masking ablation for Spagraph Stage 3 on GSE144240/GSE144236."
    )
    parser.add_argument(
        "--deconv-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(REPO_ROOT / "evaluate" / "data" / "GSE144236" / "ablation_no_masking"),
        help="Output directory for the no-masking run.",
    )
...
def main() -> None:
    ...
    print("Running SCC no-masking ablation with fixed Stage 3 settings:")
    ...
    print("  ablation:           no masking (edge/node masking and reconstruction disabled)")
