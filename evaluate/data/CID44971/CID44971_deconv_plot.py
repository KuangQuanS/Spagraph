...
# 文件路径
# ------------------------------
cell_composition_csv = DATASET_DIR / "CID44971_ST_composition.csv"
st_h5ad_path = REPO_ROOT / "spagraph_data" / "database" / "Wu" / "CID44971" / "CID44971_ST.h5ad"
output_dir = DATASET_DIR / "deconv_fig"
cosine_csv = ""

# 创建 deconv_fig 文件夹
deconv_fig_dir = output_dir
deconv_fig_dir.mkdir(parents=True, exist_ok=True)
...
ax.set_axis_off()
pdf_path = deconv_fig_dir / "dominant_cell_type.pdf"
...
for col, ct in zip(cell_comp_cols, cell_types):
    ...
    fig, ax = plt.subplots(figsize=(8, 8))
    raw_vmax = np.quantile(adata_subset.obs[col], 0.98)
    step = 0.01
    ...
    pdf_path = deconv_fig_dir / f"spatial_distribution_{ct}.pdf"
    plt.savefig(pdf_path, bbox_inches='tight', transparent=True)
    print(f"✅ Saved PDF: {pdf_path}")
    plt.close()
...
dists, _ = nbrs.kneighbors(coords)
median_nn = np.median(dists[:, 1])
pie_radius = median_nn * scale_factor * 0.4  # 调整半径 (0.45 -> 0.4)
...
for spot_idx in range(len(comp_df)):
    ...
    for val, cell_type in zip(vals, cell_types):
        ...
        wedge = Wedge((cx, cy), pie_radius, theta_start, theta_end,
                 facecolor=color_map[cell_type], edgecolor='black', # 恢复黑色边框
...
# 保存图像为PDF
output_file = deconv_fig_dir / 'spot_pie_charts_on_tissue.pdf'
...
try:
    ...
    # 保存图像为PDF
    output_file = deconv_fig_dir / 'cosine_similarity_heatmap.pdf'
    fig.savefig(output_file, bbox_inches='tight', transparent=True)
    print(f"\n✅ Saved PDF: {output_file}")
