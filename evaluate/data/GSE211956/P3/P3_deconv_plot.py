...
# 文件路径
# ------------------------------
cell_composition_csv = DATASET_DIR / "GSE211956_ST_P3_cell_composition.csv"
st_h5ad_path = REPO_ROOT / "spagraph_data" / "database" / "GSE211956" / "GSE211956_ST_P3.h5ad"
...
for ct in cell_comp.columns:
    # 将负值裁剪为 0
    adata_subset.obs[f"cell_comp_{ct}"] = np.clip(cell_comp[ct].values, 0, None)
...
for col, ct in zip(cell_comp_cols, cell_types):
    ...
    sc.pl.spatial(
        ...
        cmap="viridis",
        vmin=0,
        vmax=np.floor(np.quantile(adata_subset.obs[col], 0.98) * 10) / 10,   # 向下取整到一位小数
        title=f"{ct}",
        ax=ax,
        show=False,
        img_key="lowres"  # 使用低分辨率背景图
    )
    # 增大颜色条字体并加粗
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    # Sanitize cell type name for filename to avoid FileNotFoundError with slashes
    safe_ct = ct.replace('/', '_').replace('\\', '_')
    pdf_path = output_dir / f"spatial_distribution_{safe_ct}.pdf"
    plt.savefig(pdf_path, bbox_inches='tight', transparent=True)
    print(f"✅ Saved PDF: {pdf_path}")
    plt.close()
