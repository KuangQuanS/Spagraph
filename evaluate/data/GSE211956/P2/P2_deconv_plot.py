...
# 文件路径
# ------------------------------
cell_composition_csv = DATASET_DIR / "GSE211956_ST_P2_cell_composition.csv"
st_h5ad_path = REPO_ROOT / "spagraph_data" / "database" / "GSE211956" / "GSE211956_ST_P2.h5ad"
...
for col, ct in zip(cell_comp_cols, cell_types):
    ...
    sc.pl.spatial(
        ...
        cmap="viridis",
        vmin=0,
        vmax=np.quantile(adata_subset.obs[col], 0.98),   # 避免极端值导致图太暗
        title=f"Spatial distribution of {ct}",
        ax=ax,
        show=False,
        img_key=None  # 禁用背景图以避免 MemoryError (700MB+)
    )
