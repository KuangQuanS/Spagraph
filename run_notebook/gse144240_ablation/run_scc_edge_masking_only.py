...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run SCC edge-masking-only ablation (no node masking) for Spagraph Stage 3."
    )
    parser.add_argument(
        "--deconv-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(REPO_ROOT / "evaluate" / "data" / "GSE144236" / "ablation_edge_masking_only"),
    )
...
def main() -> None:
    ...
    print("Running SCC edge-masking-only ablation:")
    print(f"  deconv_dir:         {deconv_dir}")
    print(f"  st_h5ad:            {args.st_h5ad}")
    print(f"  output_dir:         {output_dir}")
    print(f"  ablation:           edge masking only (node_mask_ratio=0, lambda_node_recon=0)")
